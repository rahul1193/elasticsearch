 ...
 * <p>
 * Must be created using {@link InternalRestClientBuilder}, which allows to set all the different options or just rely on defaults.
...
public class InternalRestClient implements Closeable {
     ...
     */
    public static InternalRestClientBuilder builder(HttpHost... hosts) {
        return new InternalRestClientBuilder(hosts);
    }
        ...
        client.execute(requestProducer, responseConsumerFactory.createHttpAsyncResponseConsumer(), new FutureCallback<HttpResponse>() {
            ...
            public void completed(HttpResponse httpResponse) {
                try {
                    ...
                    RestResponse response = new RestResponse(request.getRequestLine(), host, httpResponse);
    ...
    static class SyncResponseListener implements ResponseListener {
         ...
         */
        RestResponse get() throws IOException {
            try {
            ...
            RestResponse response = this.response.get();
            if (exception != null) {
                if (response != null) {
